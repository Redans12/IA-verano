...
class Juego:
    def __init__(self) -> None:
        ...
        self.avanzado = False       # Indica si el personaje esta adelante o en posicion de inicio
        self.avance_offset = 40.0   # Pixeles de desplazamiento
        self.avance_timer = 0       # Contador de frames en los que el personaje se encuentra adelante
        self.avance_duracion = 20   # Frames de duracion hacia adelante  
    ...
    def _apply_resolution(self, w: int, h: int, reset_positions: bool) -> None:
        ...
        self.salto_vel = self.salto_vel_inicial
        self.avance_offset = 40 * self.scale
    ...
    def iniciar_avance(self) -> None:
        if not self.avanzado:
            self.jugador.x += self.margin + int(self.avance_offset)
            self.avanzado = True
            self.avance_timer = 0

    def manejar_avance(self) -> None:
        if self.avanzado:
            self.avance_timer += 1
            if self.avance_timer >= self.avance_duracion:
    ...
    def entrenar_modelo(self) -> Tuple[bool, str]:
        ...
        self.modelo = clf
        self.modelo_entrenado = True
        return True, f"Árbol de decisión entrenado. Accuracy test ≈ {acc:.3f}"
    ...
    def _dibujar_menu(self, msg: str = "") -> None:
        ...
        opciones = [
            ...
            "A - Auto (usa Árbol de Decisión; sin modelo NO salta)",
            "T - Entrenar Árbol de Decisión",
        ...
        while esperando and self.corriendo:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    ...
                    if e.key == pygame.K_a:
                        if not self.modelo_entrenado:
                            msg = "Primero entrena el Árbol de Decisión (T) en esta sesión."
                        else:
                            self.modo_auto = True
                            self._reset_estado_juego()
                            esperando = False
    ...
    def loop(self) -> None:
        ...
        while self.corriendo:
            ...
            for e in pygame.event.get():
                ...
                # La ventana es de tamaño fijo: ignoramos eventos VIDEORESIZE.
                    ...
                    if e.key == pygame.K_q:
                        ...
                        salto_frame = True
                        self.iniciar_salto()
                    elif e.key == pygame.K_RIGHT and (not self.modo_auto) and (not self.avanzado):                   
                        self.iniciar_avance()                    
            ...
            self.manejar_avance()  # Manejamos avance solo si estamos en salto
